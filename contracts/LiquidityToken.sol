...
contract LiquidityToken is IERC20, Ownable, BaseToken {
    ...
    uint256 public constant MAX_FEE = 10**4 / 5; // Max Fee 20%
    ...
    constructor(
        ...
        if (teamAddress_ == address(0)) {
            require(
                teamFee_ == 0,
                "Cant set both team address to address 0 and team fee percent more than 0"
            );
        }
        require(
            reflectionFee_ + liquidityFee_ + teamFee_ <= MAX_FEE,
        ...
        emit TokenCreated(owner(),address(this),TokenType.liquidityToken,VERSION);
    ...
    function setReflectionFeePercent(uint256 reflectionFee) external onlyOwner {
        ...
        require(
            _reflectionFee + _liquidityFee + _teamFee <= MAX_FEE,
            "Total fee should be less than 20%"
        );
    }
